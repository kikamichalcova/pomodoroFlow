...
const Settings = ({ visible,
  ...
  const closeSettingsAndNavigate = () => {
    toggleSettingsVisibility(); // Zrušiť nastavenia
    navigate('/pomodoro-app'); // Prejsť na pomodoro časovač
  };
  const colors = {
    default: '#F87070',
    blue:   '#70F3F8',
  ...
  const applySettings = (event) => {
    ...
    navigate('/pomodoro-app');
  }

  if (visible) {
    return (
      <div className="preferences preferences--visible">
      <div className="preferences__pane settings-pane">
      <Button type="close" buttonText="×" toggleVisibility={closeSettingsAndNavigate} />
        <h2>Settings</h2>
